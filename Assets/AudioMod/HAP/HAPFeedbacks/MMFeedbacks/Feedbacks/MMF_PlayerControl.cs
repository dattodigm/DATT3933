...
namespace XboxHaptics.Feedbacks
{
	...
	[MovedFrom(false, null, "XboxHaptics.Feedbacks")]
	...
	public class MMF_PlayerControl : MMF_Feedback
	{
		...
		public override float FeedbackDuration
		{
			get
			{
				...
				if ((Mode == Modes.PlayFeedbacks) && (TargetPlayers.Count > 0))
				{
					...
					foreach (HAP_Player player in TargetPlayers)
					{
		...
		public List<HAP_Player> TargetPlayers;
		...
		protected override void CustomInitialization(HAP_Player owner)
		{
			base.CustomInitialization(owner);
			if (TargetPlayers == null)
			{
				TargetPlayers = new List<HAP_Player>();
			}
		}
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			switch (Mode)
			{
				case Modes.PlayFeedbacks:
					foreach (HAP_Player player in TargetPlayers) { player.PlayFeedbacks(position, feedbacksIntensity); }
					break;
				case Modes.StopFeedbacks:
					foreach (HAP_Player player in TargetPlayers) { player.StopFeedbacks(); }
					break;
				case Modes.PauseFeedbacks:
					foreach (HAP_Player player in TargetPlayers) { player.PauseFeedbacks(); }
					break;
				case Modes.ResumeFeedbacks:
					foreach (HAP_Player player in TargetPlayers) { player.ResumeFeedbacks(); }
					break;
				case Modes.Initialization:
					foreach (HAP_Player player in TargetPlayers) { player.Initialization(); }
					break;
				case Modes.PlayFeedbacksInReverse:
					foreach (HAP_Player player in TargetPlayers) { player.PlayFeedbacksInReverse(position, feedbacksIntensity, true); }
					break;
				case Modes.PlayFeedbacksOnlyIfReversed:
					foreach (HAP_Player player in TargetPlayers) { player.PlayFeedbacksOnlyIfReversed(position, feedbacksIntensity); }
					break;
				case Modes.PlayFeedbacksOnlyIfNormalDirection:
					foreach (HAP_Player player in TargetPlayers) { player.PlayFeedbacksOnlyIfNormalDirection(position, feedbacksIntensity); }
					break;
				case Modes.ResetFeedbacks:
					foreach (HAP_Player player in TargetPlayers) { player.ResetFeedbacks(); }
					break;
				case Modes.ChangeDirection:
					foreach (HAP_Player player in TargetPlayers) { player.ChangeDirection(); }
					break;
				case Modes.SetDirectionTopToBottom:
					foreach (HAP_Player player in TargetPlayers) { player.SetDirectionTopToBottom(); }
					break;
				case Modes.SetDirectionBottomToTop:
					foreach (HAP_Player player in TargetPlayers) { player.SetDirectionBottomToTop(); }
					break;
				case Modes.RestoreInitialValues:
					foreach (HAP_Player player in TargetPlayers) { player.RestoreInitialValues(); }
					break;
				case Modes.SkipToTheEnd:
					foreach (HAP_Player player in TargetPlayers) { player.SkipToTheEnd(); }
					break;
				case Modes.RefreshCache:
					foreach (HAP_Player player in TargetPlayers) { player.RefreshCache(); }
					break;
				default:
