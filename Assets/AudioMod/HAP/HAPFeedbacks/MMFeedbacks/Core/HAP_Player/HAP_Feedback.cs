...
namespace XboxHaptics.Feedbacks
{
	...
	public abstract class MMF_Feedback
	{
		...
		/// right click anywhere in your project (usually in a Data folder) and go XboxHaptics > MMChannel, then name it with some unique name
		[Tooltip(
			"the MMChannel definition asset to use to broadcast this feedback. The shaker will have to reference that same MMChannel definition to receive events - to create a MMChannel, " +
			"right click anywhere in your project (usually in a Data folder) and go XboxHaptics > MMChannel, then name it with some unique name")]
		...
		[HideInInspector] public HAP_Player Owner;
		...
		public virtual void PreInitialization(HAP_Player owner, int index)
		{
		...
		/// </summary>
		/// <param name="owner"></param>
		public virtual void Initialization(HAP_Player owner, int index)
		{
			if (Timing == null)
			{
		...
		public virtual void Play(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			if (!_initialized)
			{
				string feedbackName = this.ToString().Replace("XboxHaptics.Feedbacks.", "");
			...
			get
			{
				switch (Timing.PlayDirection)
				{
					case MMFeedbackTiming.PlayDirections.FollowMMFeedbacksDirection:
						return (Owner.Direction == HAP_Player.Directions.TopToBottom);
			...
			get
			{
				...
				switch (Timing.MMFeedbacksDirectionCondition)
				{
					...
					case MMFeedbackTiming.MMFeedbacksDirectionConditions.OnlyWhenBackwards:
						return (Owner.Direction == HAP_Player.Directions.BottomToTop);
				}
		...
		/// </summary>
		/// <param name="owner"></param>
		protected virtual void CustomInitialization(HAP_Player owner) { }
