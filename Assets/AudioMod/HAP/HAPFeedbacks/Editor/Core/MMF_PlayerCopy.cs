...
namespace XboxHaptics.Feedbacks
{
	...
	static class MMF_PlayerCopy
	{
		...
		public static readonly Dictionary<HAP_Player, List<MMF_Feedback>> RuntimeChanges = new Dictionary<HAP_Player, List<MMF_Feedback>>();
		...
		{
			foreach (HAP_Player player in Object.FindObjectsByType<HAP_Player>(FindObjectsInactive.Include, FindObjectsSortMode.None).Where(p => p.KeepPlayModeChanges))
			{
		...
		{
			foreach (HAP_Player player in Object.FindObjectsByType<HAP_Player>(FindObjectsInactive.Include, FindObjectsSortMode.None).Where(MMF_PlayerCopy.RuntimeChanges.ContainsKey))
			{
		...
		static public void CopyAll(HAP_Player sourceFeedbacks)
		{
			CopiedFeedbacks.Clear();
		...
		static public void PasteAll(MMF_PlayerEditorUITK targetEditor)
		{
			...
			{
				targetEditor.targetHapPlayer.AddFeedback(feedback);
			}
			CopiedFeedbacks.Clear();
		}
		...
		static public void StoreRuntimeChanges(HAP_Player player)
		{
		...
		static public void ApplyRuntimeChanges(HAP_Player player)
		{
