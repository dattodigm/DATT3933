...
namespace XboxHaptics.Feedbacks
{
	...
	[CustomEditor(typeof(HAP_Player), true)]
	public class MMF_PlayerEditorUITK : Editor
	{
		...
		// Properties
		[FormerlySerializedAs("TargetMmfPlayer")] public HAP_Player targetHapPlayer;
		...
		protected const string _settingsText = "HAP PLAYER SETTINGS";
		...
		private void OnEditorUpdate()
		{
			...
			if (Application.isPlaying)
			{
				...
				if (targetHapPlayer.IsPlaying != _isPlayingLastFrame)
				{
					// display IsPlaying label in the top bar
					_isPlayingLabel.style.display = targetHapPlayer.IsPlaying ? DisplayStyle.Flex : DisplayStyle.None;
					if (!targetHapPlayer.IsPlaying)
					{
						foreach (var headerData in FeedbackHeaderContainersDictionary)
						{
			...
			_isPlayingLastFrame = targetHapPlayer.IsPlaying;
		}
		...
		{
			targetHapPlayer = (HAP_Player)target;
		...
		public virtual void DrawDurationAndDirectionContents()
		{
			...
			Label durationLabel = new Label("[" + targetHapPlayer.TotalDuration.ToString("F2") + "s]");
			...
			directionIcon.style.backgroundImage = (targetHapPlayer.Direction == HAP_Player.Directions.BottomToTop)
		...
		protected virtual void DrawFeedbacksInactiveWarning(VisualElement root)
		{
			...
			if (!HAP_Player.GlobalMMFeedbacksActive)
			{
		...
		protected virtual void DrawAutomaticShakerSetupButton()
		{
			...
			if (!Application.isPlaying && targetHapPlayer.HasAutomaticShakerSetup)
			{
				Button automaticShakerSetupButton = new Button(() => targetHapPlayer.AutomaticShakerSetup());
		...
		{
			int count = 0;
			if ((targetHapPlayer != null) && (targetHapPlayer.FeedbacksList != null))
			{
				count = targetHapPlayer.FeedbacksList.Count;
			}
		...
		protected virtual void DrawEmptyListState()
		{
			...
			if ((targetHapPlayer.FeedbacksList == null) || (targetHapPlayer.FeedbacksList.Count == 0))
			{
		...
		protected virtual void AssembleFeedbacksList()
		{
			...
			{
				Type feedbackType = targetHapPlayer.FeedbacksList[index].GetType();
				
				// to prevent double bindings during list reorders, we return if we've already bound this feedback
				if (FeedbackHeaderContainersDictionary.ContainsKey(targetHapPlayer.FeedbacksList[index]))
				{
					return;
				}
				...
				foldout.Clear();
				foldout.value = targetHapPlayer.FeedbacksList[index].IsExpanded;
				foldout.viewDataKey = targetHapPlayer.name + "-" + targetHapPlayer.FeedbacksList[index].UniqueID;
				...
				if (MMMenuHelp.HelpEnabled)
				{
					...
					if (!string.IsNullOrEmpty(helpText))
					{
						...
						helpBox.style.borderLeftColor = targetHapPlayer.FeedbacksList[index].FeedbackColor;
						foldout.Add(helpBox);
					}
				}
				...
				feedbackProgressBar.style.backgroundColor =
					targetHapPlayer.FeedbacksList[index].FeedbackColor.MMLighten(0.3f);
				...
				Label setupRequiredLabel = new Label(targetHapPlayer.FeedbacksList[index].RequiresSetupText);
				...
				drawData.Feedback = targetHapPlayer.FeedbacksList[index];
				...
				feedbackLeftBorder.style.backgroundColor = targetHapPlayer.FeedbacksList[index].FeedbackColor;

				// handling background color
				if (targetHapPlayer.FeedbacksList[index].DisplayFullHeaderColor)
				{
					...
					feedbackBackgroundColor.style.backgroundColor = targetHapPlayer.FeedbacksList[index].DisplayColor;
				...
				feedbackInspectorContainer.SetEnabled(targetHapPlayer.FeedbacksList[index].Active);
				...
				feedbackHeaderContainersData.Feedback = targetHapPlayer.FeedbacksList[index];
				...
				FeedbackHeaderContainersDictionary.Add(targetHapPlayer.FeedbacksList[index],
			...
			FeedbackHeaderContainersData feedbackHeaderContainersData)
		{
			...
			feedbackHeaderContainersData.SetupRequiredWarningBox.style.display =
				(targetHapPlayer.FeedbacksList[index].RequiresSetup) ? DisplayStyle.Flex : DisplayStyle.None;
			...
			// target 
			Label targetLabel = new Label(targetHapPlayer.FeedbacksList[index].RequiredTarget);
			...
			targetLabel.style.display =
				targetHapPlayer.DisplayFullDurationDetails ? DisplayStyle.None : DisplayStyle.Flex;

			// timing info 
			string timingInfo = "";
			bool displayTotal = false;
			if (targetHapPlayer.DisplayFullDurationDetails)
			{
				if (targetHapPlayer.FeedbacksList[index].Timing.InitialDelay != 0)
				{
					timingInfo +=
						targetHapPlayer.ApplyTimeMultiplier(targetHapPlayer.FeedbacksList[index].Timing.InitialDelay)
							.ToString() + "s + ";
					displayTotal = true;
				}

				timingInfo += targetHapPlayer.FeedbacksList[index].FeedbackDuration.ToString("F2") + "s";

				if (targetHapPlayer.FeedbacksList[index].Timing.NumberOfRepeats != 0)
				{
					float delayBetweenRepeats =
						targetHapPlayer.ApplyTimeMultiplier(targetHapPlayer.FeedbacksList[index].Timing
							.DelayBetweenRepeats);

					timingInfo += " + " + targetHapPlayer.FeedbacksList[index].Timing.NumberOfRepeats.ToString() +
					              " x ";
					
					timingInfo += "(" + targetHapPlayer.FeedbacksList[index].FeedbackDuration.ToString() +
					              "s + ";
				...
				if (displayTotal)
				{
					timingInfo += " = " + targetHapPlayer.FeedbacksList[index].TotalDuration.ToString("F2") + "s";
				}
			}
			else
			{
			...
			// setup required icon
			if (targetHapPlayer.FeedbacksList[index].RequiresSetup)
			{
		...
		{
			if ((targetHapPlayer.FeedbacksList[index].Timing.MMFeedbacksDirectionCondition ==
			     MMFeedbackTiming.MMFeedbacksDirectionConditions.OnlyWhenForwards)
			    || (targetHapPlayer.FeedbacksList[index].Timing.MMFeedbacksDirectionCondition ==
			        MMFeedbackTiming.MMFeedbacksDirectionConditions.OnlyWhenBackwards))
			{
				return (targetHapPlayer.FeedbacksList[index].Timing.MMFeedbacksDirectionCondition ==
				        MMFeedbackTiming.MMFeedbacksDirectionConditions.OnlyWhenBackwards)
					? new StyleBackground(BottomToTopIcon)
					: new StyleBackground(TopToBottomIcon);
			}
		...
		{
			_feedbackLabel = targetHapPlayer.FeedbacksList[index].GetLabel();
			if (targetHapPlayer.FeedbacksList[index].Label != targetHapPlayer.FeedbacksList[index].OriginalLabel)
			{
				if (targetHapPlayer.FeedbacksList[index].OriginalLabel == "")
				{
					targetHapPlayer.FeedbacksList[index].OriginalLabel =
				...
				if (targetHapPlayer.FeedbacksList[index].OriginalLabel != "")
				{
					_feedbackLabel += " <color=" + _originalLabelColor + ">(" +
					                  targetHapPlayer.FeedbacksList[index].OriginalLabel + ")</color>";
				}
			}

			if ((targetHapPlayer.FeedbacksList[index].LooperPause == true) && (Application.isPlaying))
			{
				if ((targetHapPlayer.FeedbacksList[index] as MMF_Looper).InfiniteLoop)
				{
				...
				else
				{
					_feedbackLabel = _feedbackLabel + "[" +
					                 (targetHapPlayer.FeedbacksList[index] as MMF_Looper).NumberOfLoopsLeft +
					                 " loops left] ";
				}
			}
		...
		protected virtual void AddFeedback(int feedbackIndex, PopupField<string> addNewFeedbackPopupField)
		{
			...
			serializedObject.ApplyModifiedProperties();
			PrefabUtility.RecordPrefabInstancePropertyModifications(targetHapPlayer);
		...
		{
			_feedbacksListView.itemsSource = targetHapPlayer.FeedbacksList;
		}
		...
		protected virtual void DrawDebugControls(VisualElement root)
		{
			...
			Button initializeButton = new Button(() => targetHapPlayer.Initialization()) { text = _initializeText };
			Button playButton = new Button(() => targetHapPlayer.PlayFeedbacks()) { text = _playText };
			Button pauseButton = new Button(() => targetHapPlayer.PauseFeedbacks()) { text = _pauseText };
			Button stopButton = new Button(() => targetHapPlayer.StopFeedbacks()) { text = _stopText };
			Button resetButton = new Button(() => targetHapPlayer.ResetFeedbacks()) { text = _resetText };
			...
			if (Application.isPlaying && targetHapPlayer.ContainsLoop)
			{
			...
			Button skipButton = new Button(() => targetHapPlayer.SkipToTheEnd()) { text = _skipText };
			Button restoreButton = new Button(() => targetHapPlayer.RestoreInitialValues()) { text = _restoreText };
			Button changeDirectionButton = new Button(() => targetHapPlayer.ChangeDirection())
			...
			if (targetHapPlayer.KeepPlayModeChanges)
			{
			...
			{
				targetHapPlayer.KeepPlayModeChanges = !targetHapPlayer.KeepPlayModeChanges;
				if (targetHapPlayer.KeepPlayModeChanges)
				{
					keepPlaymodeChangesButton.AddToClassList(_playModeButtonActiveClassName);
				}
				...
				EditorUtility.SetDirty(targetHapPlayer);
			});
			...
			// resume button
			Button resumeButton = new Button(() => targetHapPlayer.ResumeFeedbacks()) { text = _resumeText };
		...
		{
			return (target as HAP_Player).AddFeedback(type);
		}
		...
		protected virtual void RemoveFeedback(int id)
		{
			...
			(target as HAP_Player).RemoveFeedback(id);
			serializedObject.ApplyModifiedProperties();
			RedrawFeedbacksList();
			PrefabUtility.RecordPrefabInstancePropertyModifications(targetHapPlayer);
		}
		...
		protected virtual void ResetContextMenuFeedback(int id)
		{
			...
			Type feedbackType = (target as HAP_Player).FeedbacksList[id].GetType();
			MMF_Feedback newFeedback = (target as HAP_Player).AddFeedback(feedbackType, false);
			(target as HAP_Player).FeedbacksList[id] = newFeedback;
			serializedObject.ApplyModifiedProperties();
			RedrawFeedbacksList();
			PrefabUtility.RecordPrefabInstancePropertyModifications(targetHapPlayer);
		}
		...
		{
			MMF_Feedback feedback = targetHapPlayer.FeedbacksList[id];
			feedback.Initialization(targetHapPlayer, id);
		}
		...
		{
			MMF_Feedback feedback = targetHapPlayer.FeedbacksList[id];
			feedback.Play(targetHapPlayer.transform.position, targetHapPlayer.FeedbacksIntensity);
		}
		...
		{
			MMF_Feedback feedback = targetHapPlayer.FeedbacksList[id];
			feedback.Stop(targetHapPlayer.transform.position);
		}
		...
		{
			MMF_PlayerCopy.CopyAll(target as HAP_Player);
			RedrawBottomBar();
		}
		...
		protected virtual void ReplaceAll()
		{
			...
			targetHapPlayer.FeedbacksList.Clear();
			MMF_PlayerCopy.PasteAll(this);
			serializedObject.ApplyModifiedProperties();
			PrefabUtility.RecordPrefabInstancePropertyModifications(targetHapPlayer);
			RedrawBottomBar();
